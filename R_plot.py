...
# Calculate R^2
residuals = voltage - double_exponential_decay(time_us_scaled, *params)
ss_res = np.sum(residuals ** 2)  # Residual sum of squares
ss_tot = np.sum((voltage - np.mean(voltage)) ** 2)  # Total sum of squares
...
max_voltage_position = (time_fit_scaled[np.argmax(voltage_fit)], max(voltage_fit))
plt.annotate(
    f"$R^2 = {r_squared:.4f}$",
    xy=max_voltage_position,  # Position to point the arrow
    xytext=(max_voltage_position[0] + 0.05, max_voltage_position[1] + 0.2),  # Text position

...
def k_fold_cross_validation(X, y, k=5, alpha_values=None, random_state=None):
    ...
    for alpha in alpha_values:
        ...
        for fold in range(k):
            ...
            y_pred_val = predict(X_val, beta)
            error = mean_squared_error_manual(y_val, y_pred_val)
            errors.append(error)
...
alpha_values = np.logspace(-6, 6, 50)  # Different values of alpha for cross-validation
best_alpha = k_fold_cross_validation(X_train, y_train, k=10, alpha_values=alpha_values, random_state=42)

...
for R, C, Vcc in parameters:
    ...
    # Calculate MAPE and R-squared for generalized model
    generalized_mape = np.mean(np.abs((voltage - np.interp(time_us / 1e6, t_th, V_in)) / voltage)) * 100
    generalized_ss_total = np.sum((voltage - np.mean(voltage))**2)
    generalized_ss_residual = np.sum((voltage - np.interp(time_us / 1e6, t_th, V_in))**2)
    generalized_r_squared = 1 - (generalized_ss_residual / generalized_ss_total)
...
# Calculate MAPE and R-squared for double exponential model
double_mape = np.mean(np.abs((voltage - double_exponential_decay(time_us_scaled, *params)) / voltage)) * 100
double_ss_total = np.sum((voltage - np.mean(voltage))**2)
double_ss_residual = np.sum((voltage - double_exponential_decay(time_us_scaled, *params))**2)
double_r_squared = 1 - (double_ss_residual / double_ss_total)
...
# Annotate MAPE and R-squared on the plot
plt.text(0.175, 4.0, f"Generalized Model:\nMAPE = {generalized_mape:.2f}%\nR² = {generalized_r_squared:.4f}",
         bbox=dict(facecolor='white', alpha=0.5), fontsize=10)
plt.text(0.175, 3.5, f"Double Exp. Model:\nMAPE = {double_mape:.2f}%\nR² = {double_r_squared:.4f}",
         bbox=dict(facecolor='white', alpha=0.5), fontsize=10)
